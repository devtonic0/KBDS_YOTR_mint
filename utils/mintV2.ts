...
export const getRemainingAccountsByGuardType = ({
  ...
  payer: PublicKey
  guard: Option<SolPaymentGuardSettings | TokenPaymentGuardSettings | object>
  guardType: string
}) => {
  const remainingAccs: {
    [key: string]: () => {
    ...
    tokenPayment: () => {
      if (!candyMachine.candyGuard) return {}
      const tokenPaymentGuard = guard as TokenPaymentGuardSettings
      const SPL_ASSOCIATED_TOKEN_ACCOUNT_PROGRAM_ID = new PublicKey(
        "ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL"
      );

      const [payerTokenPda] = PublicKey.findProgramAddressSync(
        [
          payer.toBuffer(),
          TOKEN_PROGRAM_ID.toBuffer(),
          tokenPaymentGuard.mint.toBuffer(),
        ],
      ...
      return {
        accounts: [
          {
            pubkey: payerTokenPda,
            isSigner: false,
            isWritable: true,
          },
          {
            pubkey: tokenPaymentGuard.destinationAta,
            isSigner: false,
            isWritable: true,
          },
        ],
